    ...
    public partial class PKU : UserControl
    {
        ...
        public static PKU Instance
        {
             get
            {
                ...
                list = BLL.JudgeTempletBLL.GetListByName("PKU");
                if (list.Count > 0)
                {
                ...
                instance.testSheet = new Test_PKU();
        ...
        private void PKU_Load(object sender, EventArgs e)
        {
            start();
        ...
        private void BindFormlToObject()
        {
            ...
            testSheet.R241C = list.Contains("R241C") ? true : false;
            testSheet.R241R = list.Contains("R241R") ? true : false;
            testSheet.R243Q = list.Contains("R243Q") ? true : false;
            testSheet.R243R = list.Contains("R243R") ? true : false;
            testSheet.R252Q = list.Contains("R252Q") ? true : false;
            testSheet.R252R = list.Contains("R252R") ? true : false;
            testSheet.R413P = list.Contains("R413P") ? true : false;
            testSheet.R413R = list.Contains("R413R") ? true : false;
            testSheet.V245VM = list.Contains("V245VM") ? true : false;
            testSheet.V245VW = list.Contains("V245VW") ? true : false;
        ...
        private void BindObjectToForm()
        {
            ...
            #region
            if (Convert.ToBoolean(testSheet.V245VM))
            {
                ClickState("V245VM");
                list.Add("V245VM");
            }
            if (Convert.ToBoolean(testSheet.V245VW))
            {
                ClickState("V245VW");
                list.Add("V245VW");
            }
            if (Convert.ToBoolean(testSheet.R241C))
            {
                ClickState("R241C");
                list.Add("R241C");
            }
            if (Convert.ToBoolean(testSheet.R241R))
            {
                ClickState("R241R");
                list.Add("R241R");
            }
            if (Convert.ToBoolean(testSheet.R243Q))
            {
                ClickState("R243Q");
                list.Add("R243Q");
            }
            if (Convert.ToBoolean(testSheet.R243R))
            {
                ClickState("R243R");
                list.Add("R243R");
            }
            if (Convert.ToBoolean(testSheet.R252Q))
            {
                ClickState("R252Q");
                list.Add("R252Q");
            }
            if (Convert.ToBoolean(testSheet.R252R))
            {
                ClickState("R252R");
                list.Add("R252R");
            }
            if (Convert.ToBoolean(testSheet.R413P))
            {
                ClickState("R413P");
                list.Add("R413P");
            }
            if (Convert.ToBoolean(testSheet.R413R))
            {
                ClickState("R413R");
                list.Add("R413R");
            }
        ...
        private void TsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            string path = Application.StartupPath + @"\ChipPic\PKU";
            ...
            string picName = @"\ChipPic\PKU\" + DateTime.Now.ToString("yyyyMMdd") + "_" + Convert.ToString(testSheet.SampleID) + ".JPG";
            ...
            if (flag == 0)
            {
                Test_PKUBLL.Insert(testSheet); // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                list.Clear();
                list.Add(" ");
            ...
            {
                flag = 0;
                if (Test_PKUBLL.Update(testSheet) > 0) // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                {
                    ...
                    for (int i = 0; i < X; i++)       // 行 column            A1|B1  C1|D1  E1
                    {
                        for (int j = 0; j < Y; j = j + 3)  //  列 row          A2|B2    j=0 a1b1   j=2 c1|d1   j=4 e1f1
                        {
                            ...
                            if (listItem != null && listItem.Count != 0)
                            {
                                ...
                                btn.Location = new Point(12 + j / 3 * 100, 8 + i * 30);
                                btn.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Item3B;
        ...
        {
            FormSearch searcher = new FormSearch(judgeId,"PKU");
            searcher.ShowDialog(this);
            if(!string.IsNullOrEmpty(searcher.updateID))
            {
                Reset();
                testSheet = Test_PKUBLL.GetDataBySheetID(Convert.ToInt32(searcher.updateID));
                BindObjectToForm();
            }
        ...
        private void TsBtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(updateID))
            {
                ...
                //{
                    instance.testSheet = Test_PKUBLL.GetDataBySheetID(Convert.ToInt32(updateID));
                    BindObjectToForm();
                //}
            }
        }
        ...
        private void TsBtnDel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(deleteIDs))
            {
               ...
               {
                     Test_PKUBLL.Delete(deleteIDs); // 调用“业务逻辑层”的方法，删除关联对象并更新至数据库。
        ...
        private void PrintSheet(Test_PKU testSheet)
        {
            Report report = new Report();

            string fileUrl = Application.StartupPath + @"\Model\PKU.dot";
            ...
            if (!string.IsNullOrEmpty(testSheet.ChipPic))
            {
                ...
                report.InsertPicture("ChipPic", picPath, 198, 128);
            }
            ...
            string path = Application.StartupPath + @"\Document\PKU\" + DateStr;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }

            string documentName = path + @"\" + DateStr + "PKU_" + Convert.ToString(testSheet.SampleID) + ".doc";
        ...
        private void BtnPrint_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.DgvGrid.Rows.Count; i++)
            {
                if (this.DgvGrid.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    ...
                    Test_PKU testSheet = Test_PKUBLL.GetDataBySheetID(myid);
        ...
        private static void BindDataGrid(string conditions)
        {
            ...
            instance.PageBar.DataSource = Test_PKUBLL.GetPageList(instance.PageBar.PageSize, instance.PageBar.CurPage, conditions);
        ...
        private void DgvGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(objID))
            {
                ...
                list.Clear();
                list.Add(" ");
                instance.testSheet = Test_PKUBLL.GetDataBySheetID(Convert.ToInt32(objID));
                flag = 1;
