    ...
    public partial class G6PD : UserControl
    {
        ...
        public static G6PD Instance
        {
             get
            {
                ...
                list = BLL.JudgeTempletBLL.GetListByName("G6PD");
                if (list.Count > 0)
                {
                ...
                instance.testSheet = new Test_G6PD();
        ...
        private void G6PD_Load(object sender, EventArgs e)
        {
            start();
        ...
        private void BindFormlToObject()
        {
            ...
            testSheet.A95A = list.Contains("A95A") ? true : false;
            testSheet.A95G = list.Contains("A95G") ? true : false;
            testSheet.C1024C = list.Contains("C1024C") ? true : false;
            testSheet.C1024T = list.Contains("C1024T") ? true : false;
            testSheet.C1311C = list.Contains("C1311C") ? true : false;
            testSheet.C1311T = list.Contains("C1311T") ? true : false;
            testSheet.C1360C = list.Contains("C1360C") ? true : false;
            testSheet.C1360T = list.Contains("C1360T") ? true : false;
            testSheet.G1376G = list.Contains("G1376G") ? true : false;
            testSheet.G1376T = list.Contains("G1376T") ? true : false;
            testSheet.G1388A = list.Contains("G1388A") ? true : false;
            testSheet.G1388G = list.Contains("G1388G") ? true : false;
            testSheet.G871A = list.Contains("G871A") ? true : false;
            testSheet.G871G = list.Contains("G871G") ? true : false;

            testSheet.阳性对照 = list.Contains("阳性") ? true : false;
            testSheet.阴性对照 = list.Contains("阴性") ? true : false;
        ...
        private void BindObjectToForm()
        {
            ...
            #region
            if (Convert.ToBoolean(testSheet.A95A))
            {
                ClickState("A95A");
                list.Add("A95A");
            }
            if (Convert.ToBoolean(testSheet.A95G))
            {
                ClickState("A95G");
                list.Add("A95G");
            }
            if (Convert.ToBoolean(testSheet.C1024C))
            {
                ClickState("C1024C");
                list.Add("C1024C");
            }
            if (Convert.ToBoolean(testSheet.C1024T))
            {
                ClickState("C1024T");
                list.Add("C1024T");
            }
            if (Convert.ToBoolean(testSheet.C1311C))
            {
                ClickState("C1311C");
                list.Add("C1311C");
            }
            if (Convert.ToBoolean(testSheet.C1311T))
            {
                ClickState("C1311T");
                list.Add("C1311T");
            }
            if (Convert.ToBoolean(testSheet.C1360C))
            {
                ClickState("C1360C");
                list.Add("C1360C");
            }
            if (Convert.ToBoolean(testSheet.C1360T))
            {
                ClickState("C1360T");
                list.Add("C1360T");
            }
            if (Convert.ToBoolean(testSheet.G1376G))
            {
                ClickState("G1376G");
                list.Add("G1376G");
            }
            if (Convert.ToBoolean(testSheet.G1376T))
            {
                ClickState("G1376T");
                list.Add("G1376T");
            }
           
            if (Convert.ToBoolean(testSheet.阳性对照))
            {
                ClickState("阳性");
                list.Add("阳性");
            }
            if (Convert.ToBoolean(testSheet.阴性对照))
            {
                ClickState("阴性");
                list.Add("阴性");
            }
            ...
            if (flag == 1)
            {
                foreach (Control c in pnlChip.Controls)
                {
                    ...
                    if (txt.Equals(btn.Text))
                    {
                        ...
                        {
                             btn.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Probe3A;
                        }
                        else
                        {
                             btn.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Item3A;
                        }
                    }
                }
            }
        ...
        private void TsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            string path = Application.StartupPath + @"\ChipPic\G6PD";
            ...
            string picName = @"\ChipPic\G6PD\" + DateTime.Now.ToString("yyyyMMdd") + "_" + Convert.ToString(testSheet.SampleID) + ".JPG";
            ...
            if (flag == 0)
            {
                Test_G6PDBLL.Insert(testSheet); // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                list.Clear();
                list.Add(" ");
            ...
            {
                flag = 0;
                if (Test_G6PDBLL.Update(testSheet) > 0) // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                {
        ...
        private List<string> JudgeResult(List<string> listTemp,Control btn)
        {
            ...
            if (listTemp.Contains(btnValue))
            {
                if (Regex.IsMatch(btnValue, @"[\u4e00-\u9fa5]+$"))
                {
                    ...
                    {
                        btn.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Probe3B;
                        if (control is Button && control.Text == btn.Text)
                        {
                            control.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Probe3B;
                        }
                    }
                }
                else
                {
                    btn.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Item3B;
                }
            ...
            else
            {
                if (Regex.IsMatch(btnValue, @"[\u4e00-\u9fa5]+$"))
                {
                    foreach (Control control in this.pnlChip.Controls)
                    {
                        ...
                        if (control is Button && control.Text == btn.Text)
                        {
                            control.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Probe3A;
                        }
                    }
                }
                else
        ...
        private void DisplaySinoChips()
        {
                ...
                if (!String.IsNullOrEmpty(judgeChip) && Regex.IsMatch(judgeChip, @"[0-9]{1,2}[x][0-9]{1,2}$"))
                {
                    ...
                    {
                        for (int j = 0; j < Y; j = j + 3)  //  列 row          A2|B2    j=0 a1b1   j=2 c1|d1   j=4 e1f1
                        {
                            Button btn = null;
                            ...
                            if (listItem != null && listItem.Count != 0)
                            {
                                ...
                                btn.Location = new Point(8 + j / 3 * 100, 8 + i * 30);
                                btn.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Item3B;
        ...
        {
            FormSearch searcher = new FormSearch(judgeId, "G6PD");
            searcher.ShowDialog(this);
            if(!string.IsNullOrEmpty(searcher.updateID))
            {
                Reset();
                testSheet = Test_G6PDBLL.GetDataBySheetID(Convert.ToInt32(searcher.updateID));
                BindObjectToForm();
            }
        ...
        private void TsBtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(updateID))
            {
                ...
                //{
                instance.testSheet = Test_G6PDBLL.GetDataBySheetID(Convert.ToInt32(updateID));
                    BindObjectToForm();
                //}
            }
        }
        ...
        private void TsBtnDel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(deleteIDs))
            {
               ...
               {
                     Test_G6PDBLL.Delete(deleteIDs); // 调用“业务逻辑层”的方法，删除关联对象并更新至数据库。
        ...
        private void PrintSheet(Test_G6PD testSheet)
        {
            Report report = new Report();

            string fileUrl = Application.StartupPath + @"\Model\G6PD.dot";
            ...
            if (!string.IsNullOrEmpty(testSheet.ChipPic))
            {
                ...
                report.InsertPicture("ChipPic", picPath, 198, 168);
            }
            ...
            string path = Application.StartupPath + @"\Document\G6PD\" + DateStr;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }

            string documentName = path + @"\" + DateStr + "G6PD_" + Convert.ToString(testSheet.SampleID) + ".doc";
        ...
        private void BtnPrint_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.DgvGrid.Rows.Count; i++)
            {
                if (this.DgvGrid.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    ...
                    Test_G6PD testSheet = Test_G6PDBLL.GetDataBySheetID(myid);
        ...
        private static void BindDataGrid(string conditions)
        {
            ...
            instance.PageBar.DataSource = Test_G6PDBLL.GetPageList(instance.PageBar.PageSize, instance.PageBar.CurPage, conditions);
        ...
        private void DgvGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(objID))
            {
                ...
                list.Clear();
                list.Add(" ");
                instance.testSheet = Test_G6PDBLL.GetDataBySheetID(Convert.ToInt32(objID));
                flag = 1;
