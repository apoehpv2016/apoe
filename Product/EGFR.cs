    ...
    public partial class EGFR : UserControl
    {
        ...
        public static EGFR Instance
        {
             get
            {
                ...
                list = BLL.JudgeTempletBLL.GetListByName("EGFR");
                if (list.Count > 0)
                {
                ...
                instance.testSheet = new Test_EGFR();
        ...
        private void EGFR_Load(object sender, EventArgs e)
        {
            start();
        ...
        private void BindFormlToObject()
        {
            ...
            testSheet._746D1 = list.Contains("746D1") ? true : false;
            testSheet._746D2 = list.Contains("746D2") ? true : false;
            testSheet._746D3 = list.Contains("746D3") ? true : false;
            testSheet._746D4 = list.Contains("746D4") ? true : false;
            testSheet._746D5 = list.Contains("746D5") ? true : false;
            testSheet.DEL746 = list.Contains("DEL746") ? true : false;
            testSheet.G719G = list.Contains("G719G") ? true : false;
            testSheet.G719S = list.Contains("G719S") ? true : false;
            testSheet.L858L = list.Contains("L858L") ? true : false;
            testSheet.L858R = list.Contains("L858R") ? true : false;
            testSheet.T790M = list.Contains("T790M") ? true : false;
            testSheet.T790T = list.Contains("T790T") ? true : false;
        ...
        private void BindObjectToForm()
        {
            ...
            #region
            if (Convert.ToBoolean(testSheet._746D1))
            {
                ClickState("746D1");
                list.Add("746D1");
            }
            if (Convert.ToBoolean(testSheet._746D2))
            {
                ClickState("746D2");
                list.Add("746D2");
            }
            if (Convert.ToBoolean(testSheet._746D3))
            {
                ClickState("746D3");
                list.Add("746D3");
            }
            if (Convert.ToBoolean(testSheet._746D4))
            {
                ClickState("746D4");
                list.Add("746D4");
            }
            if (Convert.ToBoolean(testSheet._746D5))
            {
                ClickState("746D5");
                list.Add("746D5");
            }
            if (Convert.ToBoolean(testSheet.DEL746))
            {
                ClickState("DEL746");
                list.Add("DEL746");
            }
            if (Convert.ToBoolean(testSheet.G719G))
            {
                ClickState("G719G");
                list.Add("G719G");
            }
            if (Convert.ToBoolean(testSheet.G719S))
            {
                ClickState("G719S");
                list.Add("G719S");
            }
            if (Convert.ToBoolean(testSheet.L858L))
            {
                ClickState("L858L");
                list.Add("L858L");
            }
            if (Convert.ToBoolean(testSheet.L858R))
            {
                ClickState("L858R");
                list.Add("L858R");
            }
            if (Convert.ToBoolean(testSheet.T790M))
            {
                ClickState("T790M");
                list.Add("T790M");
            }
            if (Convert.ToBoolean(testSheet.T790T))
            {
                ClickState("T790T");
                list.Add("T790T");
            }
        ...
        private void TsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            string path = Application.StartupPath + @"\ChipPic\EGFR";
            ...
            string picName = @"\ChipPic\EGFR\" + DateTime.Now.ToString("yyyyMMdd") + "_" + Convert.ToString(testSheet.SampleID) + ".JPG";
            ...
            if (flag == 0)
            {
                Test_EGFRBLL.Insert(testSheet); // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                list.Clear();
                list.Add(" ");
            ...
            {
                flag = 0;
                if (Test_EGFRBLL.Update(testSheet) > 0) // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                {
        ...
        private void txt_Click(object sender, EventArgs e)
        {
            ...
            if (TextResult.Length > 0)
            {
                ...
                //if (txtTestResult.Text.Equals("野生型"))
                //{
                //    txtTestResult.Text = "野生型未感染";
                //}
            }
            else
            {
        ...
        {
            FormSearch searcher = new FormSearch(judgeId, "EGFR");
            searcher.ShowDialog(this);
            if(!string.IsNullOrEmpty(searcher.updateID))
            {
                Reset();
                testSheet = Test_EGFRBLL.GetDataBySheetID(Convert.ToInt32(searcher.updateID));
                BindObjectToForm();
            }
        ...
        private void TsBtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(updateID))
            {
                ...
                //{
                instance.testSheet = Test_EGFRBLL.GetDataBySheetID(Convert.ToInt32(updateID));
                    BindObjectToForm();
                //}
            }
        }
        ...
        private void TsBtnDel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(deleteIDs))
            {
               ...
               {
                     Test_EGFRBLL.Delete(deleteIDs); // 调用“业务逻辑层”的方法，删除关联对象并更新至数据库。
        ...
        private void PrintSheet(Test_EGFR testSheet)
        {
            Report report = new Report();

            string fileUrl = Application.StartupPath + @"\Model\EGFR.dot";
            ...
            if (!string.IsNullOrEmpty(testSheet.ChipPic))
            {
                ...
                report.InsertPicture("ChipPic", picPath, 160, 140);
            }
            ...
            string path = Application.StartupPath + @"\Document\EGFR\" + DateStr;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }

            string documentName = path + @"\" + DateStr + "EGFR_" + Convert.ToString(testSheet.SampleID) + ".doc";
        ...
        private void BtnPrint_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.DgvGrid.Rows.Count; i++)
            {
                if (this.DgvGrid.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    ...
                    Test_EGFR testSheet = Test_EGFRBLL.GetDataBySheetID(myid);
        ...
        private static void BindDataGrid(string conditions)
        {
            ...
            instance.PageBar.DataSource = Test_EGFRBLL.GetPageList(instance.PageBar.PageSize, instance.PageBar.CurPage, conditions);
        ...
        private void DgvGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(objID))
            {
                ...
                list.Clear();
                list.Add(" ");
                instance.testSheet = Test_EGFRBLL.GetDataBySheetID(Convert.ToInt32(objID));
                flag = 1;
