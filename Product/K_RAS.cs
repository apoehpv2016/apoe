    ...
    public partial class K_RAS : UserControl
    {
        ...
        public static K_RAS Instance
        {
             get
            {
                ...
                list = BLL.JudgeTempletBLL.GetListByName("K_ras");
                if (list.Count > 0)
                {
                ...
                instance.testSheet = new Test_K_ras();
        ...
        private void K_RAS_Load(object sender, EventArgs e)
        {
            start();
        ...
        private void BindFormlToObject()
        {
            ...
            testSheet.WT = list.Contains("WT") ? true : false;
            testSheet._12R = list.Contains("12R") ? true : false;
            testSheet._13S = list.Contains("13S") ? true : false;
            testSheet._12D = list.Contains("12D") ? true : false;
            testSheet._13D = list.Contains("13D") ? true : false;
        ...
        private void BindObjectToForm()
        {
            ...
            if (Convert.ToBoolean(testSheet.WT))
            {
                ClickState("WT");
                list.Add("WT");
            }
            if (Convert.ToBoolean(testSheet._12D))
            {
                ClickState("12D");
                list.Add("12D");
            }
            if (Convert.ToBoolean(testSheet._12R))
            {
                ClickState("12R");
                list.Add("12R");
            }
            if (Convert.ToBoolean(testSheet._13D))
            {
                ClickState("13D");
                list.Add("13D");
            }
            if (Convert.ToBoolean(testSheet._13S))
            {
                ClickState("13S");
                list.Add("13S");
            }
        ...
        private void TsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            string path = Application.StartupPath + @"\ChipPic\K_RAS";
            ...
            string picName = @"\ChipPic\K_RAS\" + DateTime.Now.ToString("yyyyMMdd") + "_" + Convert.ToString(testSheet.SampleID) + ".JPG";
            ...
            if (flag == 0)
            {
                Test_K_rasBLL.Insert(testSheet); // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                list.Clear();
                list.Add(" ");
            ...
            {
                flag = 0;
                if (Test_K_rasBLL.Update(testSheet) > 0) // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                {
        ...
        {
            FormSearch searcher = new FormSearch(judgeId,"K_ras");
            searcher.ShowDialog(this);
            if(!string.IsNullOrEmpty(searcher.updateID))
            {
                Reset();
                testSheet = Test_K_rasBLL.GetDataBySheetID(Convert.ToInt32(searcher.updateID));
                BindObjectToForm();
            }
        ...
        private void TsBtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(updateID))
            {
                ...
                //{
                    instance.testSheet = Test_K_rasBLL.GetDataBySheetID(Convert.ToInt32(updateID));
                    BindObjectToForm();
                //}
            }
        }
        ...
        private void TsBtnDel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(deleteIDs))
            {
               ...
               {
                     Test_K_rasBLL.Delete(deleteIDs); // 调用“业务逻辑层”的方法，删除关联对象并更新至数据库。
        ...
        private void PrintSheet(Test_K_ras testSheet)
        {
            Report report = new Report();

            string fileUrl = Application.StartupPath + @"\Model\K_ras.dot";
            ...
            if (!string.IsNullOrEmpty(testSheet.ChipPic))
            {
                ...
                report.InsertPicture("ChipPic", picPath, 160, 95);
            }
            ...
            string path = Application.StartupPath + @"\Document\K_RAS\" + DateStr;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }

            string documentName = path + @"\" + DateStr + "K_RAS_" + Convert.ToString(testSheet.SampleID) + ".doc";
        ...
        private void BtnPrint_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.DgvGrid.Rows.Count; i++)
            {
                if (this.DgvGrid.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    ...
                    Test_K_ras testSheet = Test_K_rasBLL.GetDataBySheetID(myid);
        ...
        private static void BindDataGrid(string conditions)
        {
            ...
            instance.PageBar.DataSource = Test_K_rasBLL.GetPageList(instance.PageBar.PageSize, instance.PageBar.CurPage, conditions);
        ...
        private void DgvGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(objID))
            {
                ...
                list.Clear();
                list.Add(" ");
                instance.testSheet = Test_K_rasBLL.GetDataBySheetID(Convert.ToInt32(objID));
                flag = 1;
