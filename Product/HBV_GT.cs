    ...
    public partial class HBV_GT : UserControl
    {
        ...
        public static HBV_GT Instance
        {
             get
            {
                ...
                list = BLL.JudgeTempletBLL.GetListByName("HBV_GT");
                if (list.Count > 0)
                {
                ...
                instance.testSheet = new Test_HBV_GT();
        ...
        private void HBV_GT_Load(object sender, EventArgs e)
        {
            start();
        ...
        private void BindFormlToObject()
        {
            ...
            testSheet.A = list.Contains("A") ? true : false;
            testSheet.B = list.Contains("B") ? true : false;
            testSheet.C = list.Contains("C") ? true : false;
            testSheet.D = list.Contains("D") ? true : false;
            testSheet.E = list.Contains("E") ? true : false;
            testSheet.F = list.Contains("F") ? true : false;
            testSheet.G = list.Contains("G") ? true : false;
            testSheet.H = list.Contains("H") ? true : false;
        ...
        private void BindObjectToForm()
        {
            ...
            #region
            if (Convert.ToBoolean(testSheet.A))
            {
                ClickState("A");
                list.Add("A");
            }
            if (Convert.ToBoolean(testSheet.B))
            {
                ClickState("B");
                list.Add("B");
            }
            if (Convert.ToBoolean(testSheet.C))
            {
                ClickState("C");
                list.Add("C");
            }
            if (Convert.ToBoolean(testSheet.D))
            {
                ClickState("D");
                list.Add("D");
            }
            if (Convert.ToBoolean(testSheet.E))
            {
                ClickState("E");
                list.Add("E");
            }
            if (Convert.ToBoolean(testSheet.F))
            {
                ClickState("F");
                list.Add("F");
            }
            if (Convert.ToBoolean(testSheet.G))
            {
                ClickState("G");
                list.Add("G");
            }
            if (Convert.ToBoolean(testSheet.H))
            {
                ClickState("H");
                list.Add("H");
            }
        ...
        private void TsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            string path = Application.StartupPath + @"\ChipPic\HBV_GT";
            ...
            string picName = @"\ChipPic\HBV_GT\" + DateTime.Now.ToString("yyyyMMdd") + "_" + Convert.ToString(testSheet.SampleID) + ".JPG";
            ...
            if (flag == 0)
            {
                Test_HBV_GTBLL.Insert(testSheet); // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                list.Clear();
                list.Add(" ");
            ...
            {
                flag = 0;
                if (Test_HBV_GTBLL.Update(testSheet) > 0) // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                {
        ...
        {
            FormSearch searcher = new FormSearch(judgeId,"HBV_GT");
            searcher.ShowDialog(this);
            if(!string.IsNullOrEmpty(searcher.updateID))
            {
                Reset();
                testSheet = Test_HBV_GTBLL.GetDataBySheetID(Convert.ToInt32(searcher.updateID));
                BindObjectToForm();
            }
        ...
        private void TsBtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(updateID))
            {
                ...
                //{
                    instance.testSheet = Test_HBV_GTBLL.GetDataBySheetID(Convert.ToInt32(updateID));
                    BindObjectToForm();
                //}
            }
        }
        ...
        private void TsBtnDel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(deleteIDs))
            {
               ...
               {
                     Test_HBV_GTBLL.Delete(deleteIDs); // 调用“业务逻辑层”的方法，删除关联对象并更新至数据库。
        ...
        private void PrintSheet(Test_HBV_GT testSheet)
        {
            Report report = new Report();

            string fileUrl = Application.StartupPath + @"\Model\HBV_GT.dot";
            ...
            if (!string.IsNullOrEmpty(testSheet.ChipPic))
            {
                ...
                report.InsertPicture("ChipPic", picPath, 155, 100);
            }
            ...
            string path = Application.StartupPath + @"\Document\HBV_GT\" + DateStr;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }

            string documentName = path + @"\" + DateStr + "HBV_GT_" + Convert.ToString(testSheet.SampleID) + ".doc";
        ...
        private void BtnPrint_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.DgvGrid.Rows.Count; i++)
            {
                if (this.DgvGrid.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    ...
                    Test_HBV_GT testSheet = Test_HBV_GTBLL.GetDataBySheetID(myid);
        ...
        private static void BindDataGrid(string conditions)
        {
            ...
            instance.PageBar.DataSource = Test_HBV_GTBLL.GetPageList(instance.PageBar.PageSize, instance.PageBar.CurPage, conditions);
        ...
        private void DgvGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(objID))
            {
                ...
                list.Clear();
                list.Add(" ");
                instance.testSheet = Test_HBV_GTBLL.GetDataBySheetID(Convert.ToInt32(objID));
                flag = 1;
