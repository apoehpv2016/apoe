    ...
    public partial class TORCH : UserControl
    {
        ...
        public static TORCH Instance
        {
             get
            {
                ...
                list = BLL.JudgeTempletBLL.GetListByName("TORCH");
                if (list.Count > 0)
                {
                ...
                instance.testSheet = new Test_TORCH();
        ...
        private void TORCH_Load(object sender, EventArgs e)
        {
            start();
        ...
        private void BindFormlToObject()
        {
            ...
            #endregion
            testSheet.CMV = list.Contains("CMV") ? true : false;
            testSheet.RV = list.Contains("RV") ? true : false;
            testSheet.TOX = list.Contains("TOX") ? true : false;
            testSheet.hsv1 = list.Contains("hsv1") ? true : false;
            testSheet.HSV2 = list.Contains("HSV2") ? true : false;
        ...
        private void BindObjectToForm()
        {
            ...
            #region
            if (Convert.ToBoolean(testSheet.hsv1))
            {
                ClickState("hsv1");
                list.Add("hsv1");
            }
            if (Convert.ToBoolean(testSheet.HSV2))
            {
                ClickState("HSV2");
                list.Add("HSV2");
            }
            if (Convert.ToBoolean(testSheet.CMV))
            {
                ClickState("CMV");
                list.Add("CMV");
            }
            if (Convert.ToBoolean(testSheet.RV))
            {
                ClickState("RV");
                list.Add("RV");
            }

            if (Convert.ToBoolean(testSheet.TOX))
            {
                ClickState("TOX");
                list.Add("TOX");
            }
        ...
        private void TsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            string path = Application.StartupPath + @"\ChipPic\TORCH";
            ...
            string picName = @"\ChipPic\TORCH\" + DateTime.Now.ToString("yyyyMMdd") + "_" + Convert.ToString(testSheet.SampleID) + ".JPG";
            ...
            if (flag == 0)
            {
                Test_TorchBLL.Insert(testSheet); // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                list.Clear();
                list.Add(" ");
            ...
            {
                flag = 0;
                if (Test_TorchBLL.Update(testSheet) > 0) // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                {
        ...
        {
            FormSearch searcher = new FormSearch(judgeId, "TORCH");
            searcher.ShowDialog(this);
            if(!string.IsNullOrEmpty(searcher.updateID))
            {
                Reset();
                testSheet = Test_TorchBLL.GetDataBySheetID(Convert.ToInt32(searcher.updateID));
                BindObjectToForm();
            }
        ...
        private void TsBtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(updateID))
            {
                ...
                //{
                instance.testSheet = Test_TorchBLL.GetDataBySheetID(Convert.ToInt32(updateID));
                    BindObjectToForm();
                //}
            }
        }
        ...
        private void TsBtnDel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(deleteIDs))
            {
               ...
               {
                     Test_TorchBLL.Delete(deleteIDs); // 调用“业务逻辑层”的方法，删除关联对象并更新至数据库。
        ...
        private void PrintSheet(Test_TORCH testSheet)
        {
            Report report = new Report();

            string fileUrl = Application.StartupPath + @"\Model\TORCH.dot";
            ...
            if (!string.IsNullOrEmpty(testSheet.ChipPic))
            {
                ...
                report.InsertPicture("ChipPic", picPath, 160, 90);
            }
            ...
            string path = Application.StartupPath + @"\Document\TORCH\" + DateStr;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }

            string documentName = path + @"\" + DateStr + "TORCH_" + Convert.ToString(testSheet.SampleID) + ".doc";
        ...
        private void BtnPrint_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.DgvGrid.Rows.Count; i++)
            {
                if (this.DgvGrid.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    ...
                    Test_TORCH testSheet = Test_TorchBLL.GetDataBySheetID(myid);
        ...
        private static void BindDataGrid(string conditions)
        {
            ...
            instance.PageBar.DataSource = Test_TorchBLL.GetPageList(instance.PageBar.PageSize, instance.PageBar.CurPage, conditions);
        ...
        private void DgvGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(objID))
            {
                ...
                list.Clear();
                list.Add(" ");
                instance.testSheet = Test_TorchBLL.GetDataBySheetID(Convert.ToInt32(objID));
                flag = 1;
