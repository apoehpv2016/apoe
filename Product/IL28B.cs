    ...
    public partial class IL28B : UserControl
    {
        ...
        public static IL28B Instance
        {
             get
            {
                ...
                list = BLL.JudgeTempletBLL.GetListByName("IL28B");
                if (list.Count > 0)
                {
                ...
                instance.testSheet = new Test_IL28B();
        ...
        private void IL28B_Load(object sender, EventArgs e)
        {
            start();
        ...
        private void InitControl()
        {
            ...
            // 绑定“性别”下拉框的数据源       BindDrpList<COL.Sex>(Sex.AllList, DrpGender, "ID", "Name");
        ...
        private void BindFormlToObject()
        {
            ...
            testSheet._9860C = list.Contains("9860C") ? true : false;
            testSheet._9860T = list.Contains("9860T") ? true : false;
            testSheet._9917G = list.Contains("9917G") ? true : false;
            testSheet._9917T = list.Contains("9917T") ? true : false;
        ...
        private void BindObjectToForm()
        {
            ...
            #region
            if (Convert.ToBoolean(testSheet._9860C))
            {
                ClickState("9860C");
                list.Add("9860C");
            }
            if (Convert.ToBoolean(testSheet._9860T))
            {
                ClickState("9860T");
                list.Add("9860T");
            }
            if (Convert.ToBoolean(testSheet._9917G))
            {
                ClickState("9917G");
                list.Add("9917G");
            }
            if (Convert.ToBoolean(testSheet._9917T))
            {
                ClickState("9917T");
                list.Add("9917T");
            }
        ...
        private void TsBtnSave_Click(object sender, EventArgs e)
        {
            ...
            string path = Application.StartupPath + @"\ChipPic\IL28B";
            ...
            string picName = @"\ChipPic\IL28B\" + DateTime.Now.ToString("yyyyMMdd") + "_" + Convert.ToString(testSheet.SampleID) + ".JPG";
            ...
            if (flag == 0)
            {
                Test_IL28BBLL.Insert(testSheet); // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                list.Clear();
                list.Add(" ");
            ...
            {
                flag = 0;
                if (Test_IL28BBLL.Update(testSheet) > 0) // 调用“业务逻辑层”的方法，检查有效性后插入至数据库。
                {
        ...
        private string[] GetString(string str, string cutStr)
        {
            ...
            IEnumerable<string> sArray = str.Split(cutChar).Distinct();
            return sArray.ToArray<string>();
        }
        ...
        private string ResultStr(String[] arry)
        {
            ...
            {
                if (arry[i].Contains("9860"))
                {
                    _9860++;
                }
                if (arry[i].Contains("9917"))
                {
                    _9917++;
                }
                if (!Regex.IsMatch(arry[i], @"[\u4e00-\u9fa5]+$"))
                {
                    ...
                    for (int j = 0; j < items.Count; j++)
                    {
                        ...
                        if (value.Equals(arry[i]))
                        {
                            ...
                            tempStr += str + ",";
                            if (str.Contains("9860") && _9860 == 2)
                            {
                                tempStr = tempStr.Replace("rs12979860位点CC", "rs12979860位点CT");
                                tempStr = tempStr.Replace("rs12979860位点TT", "rs12979860位点CT");
                            }
                            if (str.Contains("9917") && _9917 == 2)
                            {
                                tempStr = tempStr.Replace("rs8099917位点TT", "rs8099917位点TG");
                                tempStr = tempStr.Replace("rs8099917位点GG", "rs8099917位点TG");
                            }  
                        }
                    }
                }
            ...
            //if (_9860 == 2)
            //{
            //    tempStr=tempStr.Replace("9860位点CC", "9860位点CT");
            //    tempStr=tempStr.Replace("9860位点TT", "9860位点CT");
            //    _9860 = 0;

            //}
            //if (_9917 == 2)
            //{
            //    tempStr = tempStr.Replace("9917位点TT", "9917位点TG");
            //    tempStr = tempStr.Replace("9917位点GG", "9917位点TG");
            //    _9917 = 0;
            //}
            result =string.Join(",",GetString(tempStr, ","));
            return result;
        }
                    ...
                    for (int i = 0; i < X; i++)       // 行 column            A1|B1  C1|D1  E1
                    {
                        for (int j = 0; j < Y; j = j + 2)  //  列 row          A2|B2    j=0 a1b1   j=2 c1|d1   j=4 e1f1
                        {
                            ...
                            if (listItem != null && listItem.Count != 0)
                            {
                                ...
                                btn.Location = new Point(8 + j / 2 * 85, 8 + i * 30);
                                btn.BackgroundImage = MyCompany.ReportSystem.UIL.Properties.Resources.Item2B;
        ...
        {
            FormSearch searcher = new FormSearch(judgeId,"IL28B");
            searcher.ShowDialog(this);
            if(!string.IsNullOrEmpty(searcher.updateID))
            {
                Reset();
                testSheet = Test_IL28BBLL.GetDataBySheetID(Convert.ToInt32(searcher.updateID));
                BindObjectToForm();
            }
        ...
        private void TsBtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(updateID))
            {
                ...
                //{
                instance.testSheet = Test_IL28BBLL.GetDataBySheetID(Convert.ToInt32(updateID));
                    BindObjectToForm();
                //}
            }
        }
        ...
        private void TsBtnDel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(deleteIDs))
            {
               ...
               {
                   Test_IL28BBLL.Delete(deleteIDs); // 调用“业务逻辑层”的方法，删除关联对象并更新至数据库。
        ...
        private void PrintSheet(Test_IL28B testSheet)
        {
            Report report = new Report();

            string fileUrl = Application.StartupPath + @"\Model\IL28B.dot";
            ...
            if (!string.IsNullOrEmpty(testSheet.ChipPic))
            {
                ...
                report.InsertPicture("ChipPic", picPath, 160, 60);// 天津二院
               //   report.InsertPicture("ChipPic", picPath, 160, 60); 
            }
            ...
            if (!string.IsNullOrEmpty(testSheet.TestResult))
            {
                string result = testSheet.TestResult;  //CC CT TT1 TT2 TG GG
                if (result.Contains("CC"))
                    report.InsertValue("CC", "√");
                if (result.Contains("CT"))
                    report.InsertValue("CT", "√");
                if (result.Contains("rs12979860位点TT"))
                    report.InsertValue("TT1", "√");

                if (result.Contains("TG"))
                    report.InsertValue("TG", "√");
                if (result.Contains("GG"))
                    report.InsertValue("GG", "√");
                if (result.Contains("rs8099917位点TT"))
                    report.InsertValue("TT2", "√");
            }
            ...
            string path = Application.StartupPath + @"\Document\IL28B\" + DateStr;
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }

            string documentName = path + @"\" + DateStr + "IL28B_" + Convert.ToString(testSheet.SampleID) + ".doc";
        ...
        private void BtnPrint_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.DgvGrid.Rows.Count; i++)
            {
                if (this.DgvGrid.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    ...
                    Test_IL28B testSheet = Test_IL28BBLL.GetDataBySheetID(myid);
        ...
        private static void BindDataGrid(string conditions)
        {
            ...
            instance.PageBar.DataSource = Test_IL28BBLL.GetPageList(instance.PageBar.PageSize, instance.PageBar.CurPage, conditions);
        ...
        private void DgvGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(objID))
            {
                ...
                list.Clear();
                list.Add(" ");
                instance.testSheet = Test_IL28BBLL.GetDataBySheetID(Convert.ToInt32(objID));
                flag = 1;
        ...
        //    }
        //    ids = ids.TrimEnd(',');
        //    FormPrint crReport = new FormPrint("K_ras",judgeId, ids);
        //    crReport.ShowDialog();
        //}
    }
}
